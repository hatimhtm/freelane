...
import { MetricDetail, type MetricData } from "./_components/metric-detail";

const DAY_MS = 86_400_000;

const KEYS = ["landed", "outstanding", "fees", "avg-days", "debtor"] as const;
...
export default async function MetricPage({ params }: { params: Promise<{ key: string }> }) {
  ...
  switch (key) {
    case "landed": {
      ...
      // Income by client (top 5 + Other) — same shape as the dashboard donut.
      ...
      // Income by currency (this month, by project currency).
    ...
    case "fees": {
      ...
      // Fees by chain — full leaderboard (effective % + total fee).
      ...
      // Highest-fee individual payments.
    ...
    case "avg-days": {
      // quote → first payment for paid projects.
      ...
      // Slowest clients — average lag per client.
    ...
    case "debtor": {
      // Clients ranked by outstanding base, each with their open projects.
      ...
      data = { key, currency, debtor: { debtors } };
      break;
    }
  ...
  return (
    <MetricDetail data={data} title={META[key].title} description={META[key].description} />
  );
}
